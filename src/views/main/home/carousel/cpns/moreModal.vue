<template>
  <div>
    <el-dialog v-model="centerDialogVisible" title="添加轮播图" width="500" center class="modal">
      <div class="form">
...
const customUploadRequest = () => async (option) => {
  await fileUploadStore.uploadFileAction(option.file, 'video')
...
const confirmClick = async () => {
  ...
  try {
    await carouselStore.addCarouselImageAction({
      name: formData.name,
      url: formData.url,
